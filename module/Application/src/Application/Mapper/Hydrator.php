...
class Hydrator extends Reflection
{
    ...
    public function __construct(AbstractMapper $_mapper)
    {
        ...
        $this->_table = $_mapper->computeTable();
    ...
    public function initMap(array $_attrs, array $_keys)
    {
        ...
        if (count($_keys) > 1) {
            ...
            foreach ($_attrs as $attr) {
                $attrs[] = in_array($attr, $_keys) ? [$attr => '_id'] : $attr;
            }
        }

        static::$_map[$this->_table] = [];
        ...
        foreach ($attrs as $attr) {
            if (is_array($attr)) {
                ...
                static::$_map[$this->_table][$attr] = '_'.self::upperCase($attr, true);
            }
        }
    }
    ...
    {
        $res = str_replace(' ', '', ucwords(implode(' ', AbstractMapper::split($_s))));
